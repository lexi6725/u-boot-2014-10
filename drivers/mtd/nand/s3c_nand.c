...
{
	unsigned int cur;
	struct s3c6410_nand *nand_regs = s3c6410_get_base_nand();
	...
	{
	    if (ctrl & NAND_CLE)
	        writeb(dat, nand_regs->nfcmd);
	    else if (ctrl & NAND_ALE)
	        writeb(dat, nand_regs->nfaddr);
	}
}
...
{
	struct s3c6410_nand *nand_regs = s3c6410_get_base_nand();
	while (!(readl(nand_regs->nfstat) & NFSTAT_RnB))
	{
...
#if defined(CONFIG_SYS_S3C6410_NAND_HWECC)
#if 0
/*
...
{
	struct s3c6410_nand *nand_regs = s3c6410_get_base_nand();
	while (!(readl(nand_regs->nfstat) & NFSTAT_ECCENCDONE)) {}
}
...
{
	struct s3c6410_nand *nand_regs = s3c6410_get_base_nand();
	while (!(readl(nand_regs->nfstat) & NFSTAT_ECCDECDONE)) {}
}
...
{
	struct s3c6410_nand *nand_regs = s3c6410_get_base_nand();
	while (readl(nand_regs->nfestat0) & NFESTAT0_ECCBUSY) {}
}
...
static void s3c_nand_enable_hwecc(struct mtd_info *mtd, int mode)
{
	...
	writel(nfconf, nand_regs->nfconf);

	/* Initialize & unlock */
	nfcont = readl(nand_regs->nfcont);
	...
	writel(nfcont, nand_regs->nfcont);
}
...
static int s3c_nand_calculate_ecc(struct mtd_info *mtd, const u_char *dat, u_char *ecc_code)
{
	...
	if (nand_type == S3C_NAND_TYPE_SLC) {
		nfmecc0 = readl(nand_regs->nfmecc0);
		...
		if (cur_ecc_mode == NAND_ECC_READ)
			...
			nfmecc0 = readl(nand_regs->nfmecc0);
			nfmecc1 = readl(nand_regs->nfmecc0);
...
static int s3c_nand_correct_data(struct mtd_info *mtd, u_char *dat, u_char *read_ecc, u_char *calc_ecc)
{
	...
	if (nand_type == S3C_NAND_TYPE_SLC)
	{
	    ...
	    writel(nfmeccdata0, nand_regs->nfmeccd0);
	    writel(nfmeccdata1, nand_regs->nfmeccd1);

	    /* Read ecc status */
	    nfestat0 = readl(nand_regs->nfestat0);
	...
	} else
	{
	    ...
	    nfestat0 = readl(nand_regs->nfestat0);
	    nfestat1 = readl(nand_regs->nfestat1);
	    nfmlcbitpt = readl(nand_regs->nfmlcbitpt);
...
{
	struct s3c6410_nand *nand_regs = s3c6410_get_base_nand();
	while (readl(nand_regs->nf8eccerr0) & NFESTAT0_ECCBUSY) {}
}
...
int s3c_nand_calculate_ecc_8bit(struct mtd_info *mtd, const u_char *dat, u_char *ecc_code)
{
	...
	if (cur_ecc_mode == NAND_ECC_READ)
		...
		nfm8ecc0 = readl(nand_regs->nfm8ecc0);
		nfm8ecc1 = readl(nand_regs->nfm8ecc1);
		nfm8ecc2 = readl(nand_regs->nfm8ecc2);
		nfm8ecc3 = readl(nand_regs->nfm8ecc3);
...
int s3c_nand_correct_data_8bit(struct mtd_info *mtd, u_char *dat, u_char *read_ecc, u_char *calc_ecc)
{
	...
	nf8eccerr0 = readl(nand_regs->nf8eccerr0);
	nf8eccerr1 = readl(nand_regs->nf8eccerr1);
	nf8eccerr2 = readl(nand_regs->nf8eccerr2);
	nfmlc8bitpt0 = readl(nand_regs->nfmlc8bitpt0);
	nfmlc8bitpt1 = readl(nand_regs->nfmlc8bitpt1);
...
int board_nand_init(struct nand_chip *nand)
{
	...
	if (nand_regs->nfconf & 0x80000000)
		boot_nand = 1;
	else
		boot_nand = 0;

	nand_regs->nfconf 		&= ~NFCONT_WP;
	nand->IO_ADDR_R		= (void __iomem *)(&nand_regs->nfdata);
	nand->IO_ADDR_W		= (void __iomem *)(&nand_regs->nfdata);
	...
	nand->bits_per_cell = readb(nand->IO_ADDR_R);	/* 3rd byte */
	...
	else
	{
		...
		int childType= nand->bits_per_cell; //Page size
